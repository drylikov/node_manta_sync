...
`manta_sync` requires 2 arguments, the first is a local directory that you
...
If you supply `-r`, `manta_sync` will work in reverse, pulling files from
manta onto your local filesystem.

    manta_sync -r ~~/stor/foo ./foo

Usage
-----

    $ manta_sync -h
    usage: manta_sync [OPTIONS] localdir ~~/remotedir
    ...
    examples
      ...
      manta_sync --dry-run ./ ~~/stor/foo
        - same as above, but just HEAD the data, don't PUT

      manta_sync -r ~~/stor/foo ./bar
...
`manta_sync` detected one of the files on the local end was a different
...
`manta_sync` works in 4 (optionally 5) stages
...
cause `manta_sync` to skip all files beginning with `.git/`.
...
modified with the `-m` or `--md5` switch, which tells `manta_sync` to use the md5 hash
...
`manta_sync` prints how many files were uploaded, and how many (if any) files failed
